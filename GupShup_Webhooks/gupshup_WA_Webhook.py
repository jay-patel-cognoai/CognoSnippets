...
def html_tags_formatter(message):
    ...
    if "</a>" in message:
        ...
        for link in soup.findAll('a'):
            ...
            logger.info("=== Link Found ===", extra=log_param)    
            logger.info(str(link_name), extra=log_param)    
            logger.info(str(href), extra=log_param)    
            if link_name.replace("http://","").replace("https://","").strip() == href.replace("http://","").replace("https://","").strip():
                message = message.replace(link_element, href+"\n")
            else:
...
def get_message_from_reverse_whatsapp_mapping(user_message, mobile):
    ...
    try:
        ...
        logger.info("get_message_from_reverse_whatsapp_mapping Data Dictionary: %s", str(data_dict), extra=log_param)
        if str(user_message) in data_dict:
            message = data_dict[str(user_message)]
        logger.info(str(message), extra=log_param)
    except Exception as E:
        exc_type, exc_obj, exc_tb = sys.exc_info()
        logger.error("get_message_from_reverse_whatsapp_mapping: %s", str(E),  extra=log_param)
    return message
...
def get_whatsapp_file_attachment(userid, password, request_packet):
    ...
    try:
        ...
        remote_url = "https://unify.smsgupshup.com/WhatsApp/apis/getMedia.php?userid="+str(userid)+"&password="+str(password)+"&mediaid="+str(signature)
...
def whatsapp_webhook(request_packet):
    ...
    #   GUPSHUP DELIVERY STATUS:
        if "meta" in request_packet.keys() or "statuses" in request_packet.keys():
            logger.info("INCOMING_DELIVERY_PACKET WA WEBHOOK: %s",str(request_packet), extra=log_param)
            try:
    ...
    #   WEBHOOK INIT
        ...
        logger.info("GUPSHUP WA WEBHOOK INCOMING_REQUEST_PACKET: %s",str(request_packet), extra=log_param)
    ...
    #   GET USER ATTACHMENTS AND MESSAGES:
        ...
        if is_attachement:
            ...
            filepath = get_whatsapp_file_attachment(AUTHENTICATION_NUMBER, AUTHENTICATION_KEY, attachment_packet)
    ...
    #   WEBHOOK EXECUTE FUNCTION CALL
        channel_params = {"user_mobile":mobile, "bot_number":waNumber, "whatsapp_file_attachment":filepath, "whatsapp_file_caption":file_caption, "is_new_user":first_time_user, "QueryChannel":"WhatsApp", "is_go_back_enabled": is_go_back_enabled}
        logger.info("GupShupCallBack Channel params:  %s",str(channel_params), extra=log_param)
    ...
    #   EMOJI NUMBERS: 1 to 9
        emojized_numbers = [":one:", ":two:", ":three:", ":four:",":five:",":six:", ":seven:", ":eight:", ":nine:"]
    ...
    #   CHOICES:
        ...
        if choice_display_list != []:
            ...
            for index in range(len(choice_display_list)):
                ...
                choice_str += "Enter *"+str(index+1)+"* :point_right:  "+str(choice_display_list[index])+"\n\n"
        ...
        if recommendations!= []:
            ...
            if choice_str!="":
                for index in range(len(recommendations)):
                    ...
                    recommendation_str += "Enter *"+str(reverse_mapping_index+index+1)+"* :point_right:  "+str(recommendations[index])+"\n\n"
                if is_go_back_enabled:
                    REVERSE_WHATSAPP_MESSAGE_DICT[ str(reverse_mapping_index+len(recommendations)+1) ]= "Go Back"
                    recommendation_str += "Enter *"+str(reverse_mapping_index+len(recommendations)+1)+"* :point_right:  Go Back \n\n"
                ...
                if is_go_back_enabled:
                    REVERSE_WHATSAPP_MESSAGE_DICT[ str( len(recommendations)+1) ]= "Go Back"
                    recommendation_str += "Enter *"+str( len(recommendations)+1)+"* :point_right:  Go Back \n\n"
            ...
            if is_go_back_enabled:
                ...
                recommendation_str += "Enter *"+str( le+1)+"* :point_right:  Go Back \n\n"
                recommendation_str = get_emojized_message(recommendation_str)        
        ...
        if is_single_message:
            ...
            if final_text_message!= "":
                if "http://" in final_text_message or "https://" in final_text_message :
                    ...
                    send_status = sendWhatsAppTextMessage(AUTHENTICATION_NUMBER, AUTHENTICATION_KEY, final_text_message, str(mobile),  preview_url="true")
                    time.sleep(2)
                    is_sandwich_choice = True
                else:
                    send_status = sendWhatsAppTextMessage(AUTHENTICATION_NUMBER, AUTHENTICATION_KEY, final_text_message, str(mobile), is_unicode_text=True)
                    is_sandwich_choice = True
                    time.sleep(2)
                recommendation_str = ""
                choice_str = ""
        ...
        if len(cards) > 0:
            ...
            for i in range(len(videos)):
                ...
                if 'video_link' in str(videos[i]):
                    video_url = youtube_link_formatter(str(videos[i]['video_link']))
                    send_status = sendWhatsAppTextMessage(AUTHENTICATION_NUMBER, AUTHENTICATION_KEY, video_url, str(mobile), preview_url="true")                        
            ...
            if "$$$" in choice_str:
                for bubble in choice_str.split("$$$"):
                    if bubble.strip() == "":
                        continue
                    # time.sleep(2)
                    send_status = sendWhatsAppTextMessage(AUTHENTICATION_NUMBER, AUTHENTICATION_KEY, bubble, str(mobile), is_unicode_text=True)                
                logger.info("Is choices sent: %s", str(send_status), extra=log_param)
            else:
                time.sleep(1)
                send_status = sendWhatsAppTextMessage(AUTHENTICATION_NUMBER, AUTHENTICATION_KEY, choice_str, str(mobile), is_unicode_text=True)
                logger.info("Is choices sent: %s", str(send_status), extra=log_param)
    ...
    #   SENDING RECOMMENDATIONS:
        if len(recommendation_str)>0:
            ...
            if "$$$" in recommendation_str:
                ...
                time.sleep(1)
                send_status = sendWhatsAppTextMessage(AUTHENTICATION_NUMBER, AUTHENTICATION_KEY, recommendation_str, str(mobile), is_unicode_text=True)
                logger.info("Is recommendations sent: %s", str(send_status), extra=log_param)


        logger.info("REVERSE_WHATSAPP_MESSAGE_DICT: %s", str(REVERSE_WHATSAPP_MESSAGE_DICT), extra=log_param)
        save_data(user, json_data={"REVERSE_WHATSAPP_MESSAGE_DICT":REVERSE_WHATSAPP_MESSAGE_DICT},  src="None", channel="WhatsApp", bot_id=BOT_ID,  is_cache=True)

        
        response["status_code"] = 200
        response["status_message"] = "Request processed successfully."
        logger.info("GubShupWhatsAppCallBackURL: %s",str(response), extra=log_param)
        return response    
    except Exception as E:
        exc_type, exc_obj, exc_tb = sys.exc_info()
        response["status_message"] = str(E) + str(exc_tb.tb_lineno)
        logger.error("GubShupWhatsAppCallBackURL: %s at %s", str(E), str(exc_tb.tb_lineno), extra=log_param)
        return response
