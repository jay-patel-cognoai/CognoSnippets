...
def GET_API_KEY(username,password):
    ...
    try:
        ...
        exc_type, exc_obj, exc_tb = sys.exc_info()
        logger.error("RouteMobile_GET_API_KEY Timeout error: %s", str(RT), extra=log_param)
    except Exception as E:
        exc_type, exc_obj, exc_tb = sys.exc_info()
...
def whatsapp_webhook(request_packet):
    ...
    #   WEBHOOK INIT
        ...
        logger.info("ROUTE MOBILE WA WEBHOOK INCOMING_REQUEST_PACKET: %s",str(request_packet), extra=log_param)
    ...
    # == CREDENTIALS AND DETAILS: ==============================================
        ...
        sender = remo_html_from_string(str(request_packet["messages"][0]["from"]))
    ...
    #   GET USER ATTACHMENTS AND MESSAGES:
        ...
        if is_attachement:
            ...
            filepath = get_whatsapp_file_attachment(API_KEY, attachment_packet, attachment_type)
            message = "attachment"
        else:
    ...
    #   WEBHOOK EXECUTE FUNCTION CALL
        ...
        logger.info("RouteMobileCallBack Channel params:  %s",str(channel_params), extra=log_param)
        whatsapp_response = execute_query(mobile, BOT_ID, "uat", message, "en", "WhatsApp", json.dumps(channel_params), message)
        logger.error("RouteMobileCallBack execute_query response %s",str(whatsapp_response), extra=log_param)            
    ...
    #   Check LiveChat:
        if "is_livechat" in whatsapp_response and whatsapp_response["is_livechat"] == "true":
            if message.lower() == "end chat":
                ...
                send_status = sendWhatsAppTextMessage(API_KEY, livechat_notification, mobile, preview_url=False)
        ...
        if len(cards) > 0:
            ...
            if "card_with_document" in modes.keys() and modes["card_with_document"] == "true":
                ...
                for card in cards:
                    ...
                    try:
                        ...
                        send_status = sendWhatsAppMediaMessage(API_KEY, "document", str(doc_url), mobile, caption=str(doc_caption))
                ...
                for card in cards:
                    ...
                    if image_url !="":
                        ...
                        send_status = sendWhatsAppMediaMessage(API_KEY, 'image', str(image_url), mobile, caption=caption)
                        logger.info("Is Card with image sent: %s", str(send_status), extra=log_param)
                    else:
                        send_status = sendWhatsAppTextMessage(API_KEY, get_emojized_message(caption), mobile, preview_url=True)
            ...
            for i in range(len(videos)):
                ...
                if 'video_link' in str(videos[i]):
                    ...
                    send_status = sendWhatsAppTextMessage(API_KEY, video_url, mobile, preview_url=True)
            ...
            for i in range(len(images)):
                if 'content' in str(images[i]):
                    ...
                    send_status = sendWhatsAppMediaMessage(API_KEY, 'image', str(images[i]["img_url"]), mobile, caption=str(images[i]['content']))
                    logger.info("Is Image sent: %s", str(send_status), extra=log_param)
                else:
                    logger.info("== Image without caption ==")
                    send_status = sendWhatsAppMediaMessage(API_KEY, 'image', str(images[i]), mobile, caption=None)
                    logger.info("Is Image sent: %s", str(send_status), extra=log_param)
    ...
    #   SENDING RECOMMENDATIONS:
        ...
        exc_type, exc_obj, exc_tb = sys.exc_info()
        response["status_message"] = str(E) + str(exc_tb.tb_lineno)
        logger.error("RouteMobileCallBack: %s at %s", str(E), str(exc_tb.tb_lineno), extra=log_param)
        return response
