...
def f(x):
    ...
    """
    x = json.loads(x)
    logger.info("wa livechat request packet: %s", str(x), extra={'AppName': 'EasyChat', 'user_id': "None", 'source': "None", 'channel': "None", 'bot_id': "1"})
    try:
        ...
        if x["type"] == 'text' and x['path'] == '':
            ...
            logger.info("Agent Message: %s", str(message), extra={'AppName': 'EasyChat', 'user_id': user_id, 'source': "None", 'channel': "None", 'bot_id': "1"})
            sendWhatsAppTextMessage(str(message), user_id)
        else:
            file_path = settings.EASYCHAT_HOST_URL + x["path"]
            logger.info("Agent Attachment: %s", str(file_path), extra={'AppName': 'EasyChat', 'user_id': user_id, 'source': "None", 'channel': "None", 'bot_id': "1"})
